      ...
      {/* Used this site for reference: https://codesandbox.io/s/heatmap-bangalore-forked-t57mz?file=/src/map.js:540-850
          ...
          while (i < results.length) {
            ...
            && results[i].month == month) {
                <Marker position={[x_cood, y_coord]} icon={new Icon({iconUrl: markerIconPng, iconSize: [25,41], iconAnchor: [12, 41]})}>
                  <Popup>
                    Longitude: results[i].x_coord <br /> Latitute: results[i].y_coord <br /> Average Delay (in seconds): results[i].delays.
                  </Popup>
                </Marker>
            }
            i++;
